...
//        MaxWccSzList.push_back((double)disjoint_Set.maxRankCount / (double)graph->num_nodes);
        totalMaxNum += (double)disjoint_Set.CCDScore * solution_weights;
        MaxWccSzList.push_back((double)disjoint_Set.CCDScore / norm);
    }
...
//    totalMaxNum = totalMaxNum - disjoint_Set.maxRankCount;
    ...
    return (double)totalMaxNum/((double)graph->total_nodes_weight * norm);
}
